...
const logicBlock = content.substring(componentStart + 'const BriefForm: React.FC<BriefFormProps> = ({ selectedPackage, onClearPackage, onUpgradePackage }) => {'.length, returnStart).trim();
...
const jsxPart = content.substring(returnStart);
